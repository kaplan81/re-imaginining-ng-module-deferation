...
import { widgets } from '../../../orders/orders.widgets';
 ...
 * `./Widgets`, served at :4202/widgets.
 *
 * It exists so the descriptor list has a consumer inside this project. Without
 * it, `orders.widgets.ts` is only ever exercised by the shell, and a broken
 ...
 */
@Component({
  selector: 'ord-widget-gallery',
  imports: [NgComponentOutlet],
  template: `
    <section class="ord-gallery">
      <h1 class="ord-gallery__title">Widgets published by this remote</h1>

      <p class="ord-gallery__lead">
        The same descriptors the shell mounts through <code>./Widgets</code>, rendered here by the
        roast orders remote itself. Each one gets its own environment injector built from the
        providers its descriptor ships.
      </p>

      @for (widget of mounted(); track widget.id) {
        <article class="ord-gallery__item">
          <h2 class="ord-gallery__item-title">
        ...
        <p class="ord-gallery__loading" role="status" aria-live="polite">Loading widgets…</p>
      }
    </section>
  `,
  styles: `
